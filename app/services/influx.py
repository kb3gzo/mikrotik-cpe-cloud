...
Design §5.4 defines three measurements — ``system``, ``interface``, and
``client``. Phase 1 (Task #22) only writes ``system`` with ``uptime_sec``,
because that's all the current ``telemetry-*.rsc.j2`` templates send.
Deliverable #8 expands the RouterOS scrape to include full interface and
client registration data, at which point this module grows ``interface``
and ``client`` point builders.

All exceptions from the Influx client are caught internally. Design §5.3
is explicit: telemetry ingest (the 204 response path) must not depend on
Influx being up. A flaky or offline Influx must NOT bubble up as a 500
to the router — the router would then retry-spam and the liveness
timestamp would stop updating, which is exactly the signal we need to
keep working.
"""
...
# We don't need sub-second resolution; :local uptime strips it.
...
async def get_client() -> InfluxDBClientAsync | None:
    ...
    Returns ``None`` when no token is configured — that's the dev/test
    escape hatch. Callers should treat a ``None`` return as "Influx is
    disabled for this process" and skip the write.
    """
    global _client
...
def _build_system_point(
    ...
    """Build a ``system`` measurement point per design §5.4.

    Tags: ``{router_id, model, wifi_stack}``.
    Fields (Phase 1): ``uptime_sec``. Deliverable #8 adds ``cpu_load_pct``,
    ``free_memory_bytes``, ``total_memory_bytes``, ``temperature_c``,
    ``voltage_v``.
    """
    return (
        Point("system")
...
async def write_telemetry(router: Router, payload: dict[str, Any]) -> None:
    ...
    Phase 1 behaviour: extract ``uptime``, parse to seconds, write one
    ``system`` point. Any exception from the Influx client path is logged
    and swallowed — the 204 response path is authoritative for "router
    heartbeat received" (we've already updated ``last_seen_at`` in
    ...
    try:
        ...
        uptime_sec = _parse_uptime(str(payload.get("uptime", "")))
        point = _build_system_point(router, uptime_sec, ts)
        settings = get_settings()
        write_api = client.write_api()
        await write_api.write(bucket=settings.influx_bucket, record=point)
        log.debug(
            "influx: wrote system point router_id=%s uptime_sec=%s",
            router.id,
            uptime_sec,
        )
